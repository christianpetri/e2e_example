...
export default defineComponent({
  ...
  methods: {
    async fetchData(): Promise<void> {
      ...
      try {
        const publicDataEndpoint =
          process.env.VUE_APP_PUBLIC_DATA_ENDPOINT || "/api/public-data";
        console.log(`Fetching public data from: ${publicDataEndpoint}`);
        const response = await axios.get<BackendDataResponse>(
          publicDataEndpoint
        );
    ...
    async fetchProtectedData(): Promise<void> {
      ...
      try {
        const protectedDataEndpoint =
          process.env.VUE_APP_PROTECTED_DATA_ENDPOINT || "/api/data";
    ...
    async checkBackendStatus(): Promise<void> {
      ...
      try {
        const response = await axios.get<{ message: string }>("/api/status");
