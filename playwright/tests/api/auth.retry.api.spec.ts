...
test.describe('API Client Token Retry Handling', () => {
  test.beforeAll(() => {
    ...
    const storageFile = path.resolve(__dirname, '../../auth/storageState.json');
    const raw = fs.readFileSync(storageFile, 'utf-8');
    const state = JSON.parse(raw);

    const frontendOrigin = state.origins.find((o: any) =>
      o.origin === process.env.FRONTEND_BASE_URL
    );

    const tokenEntry = frontendOrigin?.localStorage.find((i: any) => i.name === 'bearerToken');
    if (tokenEntry) {
  ...
  test('Retry on 401 with refreshed token', async () => {
    ...
    expect(body.message).toContain('This is protected data from the Java backend!');
