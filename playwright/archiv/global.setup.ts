...
async function globalSetup() {
  ...
  try {
    ...
    console.error(`ERROR: Failed to create or write to authentication directory '${AUTH_DIR}'.`);
    console.error(`Please check permissions for this path. Original error: ${dirError}`);
    throw dirError;
  }
  // --- END NEW CHECK ---
  ...
  try {
    ...
    await page.goto(FRONTEND_BASE_URL_HARDCODED, { waitUntil: 'domcontentloaded', timeout: 30000 });
    ...
    await expect(page.locator('[data-testid="login-password-input"]')).toBeVisible({ timeout: 10000 });
    ...
    await page.locator('[data-testid="login-username-input"]').fill(TEST_USERNAME_HARDCODED);
    await page.locator('[data-testid="login-password-input"]').fill(TEST_PASSWORD_HARDCODED);
    ...
    await expect(page.locator('button:has-text("Fetch Protected Data (Requires Login)")')).toBeEnabled({ timeout: 20000 });
    console.log('UI Login confirmed: "Fetch Protected Data" button is enabled.');
    await page.screenshot({ path: 'playwright-setup-3-after-login-success.png' });
    ...
    console.error('Playwright Global setup failed during login or browser interaction:', error);
    ...
    if (page && !page.isClosed()) {
        ...
        console.log('Screenshot on failure saved to playwright-setup-failure-final.png');
        ...
        try {
            ...
            await tempPage.screenshot({ path: 'playwright-setup-failure-fallback.png' });
            await tempPage.close();
            console.log('Fallback screenshot on failure saved to playwright-setup-failure-fallback.png');
