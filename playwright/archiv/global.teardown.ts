...
async function globalTeardown() {
  ...
  const isTomcatRunning = execSync(
    `docker ps -q -f name=${tomcatContainerName}`,
    { stdio: "pipe" },
  )
    .toString()
    .trim();
  ...
  if (isTomcatRunning) {
    console.log(
      `Detected running Docker container '${tomcatContainerName}'. Attempting to stop services.`,
    );
    try {
    ...
    console.log(
      `Docker container '${tomcatContainerName}' not found running. Skipping 'docker compose down'.`,
    );
  }
